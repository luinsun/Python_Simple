...
# 4) 논리형(boolean): True, False (앞에 대문자 주의)

# 좋은 개발자! : 누구나 알아보기 쉽게 코드를 작성하는 사람
# 설명: 동일한 Type에서 다양한 동류의 자료형을 사용하는 이유?
# - 메모리(저장공간)을 효율적으로 사용하기 위해서!
# - 대부분 만들어진지 오래 된 어어는 다양한 종류의 자료형 사용!
# - 자료형은 저장 할 수 있는 크기의 범위가 지정
# - 가정: int(-10000~ 10000)
# - 가정: short(-100 ~ 100)
# - 특정 값: 0~9 → 어떤 Type을 사용하면 효율적일까요? short

# 3.동적 타이핑 언어(Dynamic Typeing language)
# - Jave: int num = 4;
...
# 4.형 변환(Type Casting)
# - Type casting을 사용 하면 값을 원하는 자료형으로 변화 가능
# - 1)int(): 정수형으로 변환
# - 2)float(): 실수형으로 변환
# - 3)str(): 문자열형으로 변환
print("="*50)
...
#   * 대입연산자(=): 우측의 값을 좌측에 저장
#   * 동등연산자(==): Equal
#   * 초기화: 초기 변수를 생성 하면 쓰레기 파일들이 존재
#            변수에 값을 대입하면 초기화 값만 저장!