...
# ** selenium 사용 방법 2가지
#   1.직접 다운로드
#    - URL: https://sites.google.com/chromeium.org/driver/
#   2. 실시간 다운로드
...
def review_collector(movie_code):
    ...
    for item in review_list:
        ...
        #     reg_hour = int(re.sub(r"[^~0-9]", "", review_date))
        #     # 3) 등록일자 = 현재시간 - 17
        #     # print(f"현재 시간: {datetime.now()}")  # 년월일시분초나노초
        #     review_date = datetime.now() - timedelta(hours=reg_hour)
        #     # print(f"등록시간: {review_date}")
        #     # 4) 계산된 등록일자 날짜 포매 변경(다음 영화 리뷰 날짜 포맷)
        #     review_date = review_date.strftime("%Y. %m. %d. %H:%M")
        # print(f"   - 날짜: {review_date}")


        # MariaDB저장(제목, 리뷰, 평점, 작성자, 작성일자)
        data = {
            "title": movie_title,
